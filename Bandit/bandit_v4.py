...
def extract_parameters(nhm_params, hru_order_subset,
...
def main():
    ...
    for kk, vv in args.__dict__.items():
        if kk not in ['job', 'verbose', 'cbh_netcdf', 'add_gages', 'param_netcdf', 'no_filter_params',
                      ...
                      'prms_version']:
            if vv:
                bandit_log.info(f'Overriding configuration for {kk} with {vv} from command line.')
