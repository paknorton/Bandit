...
def model_extraction(nhm_params, check_dag=False, dsmost_seg=None, hru_noroute=None,
                     keep_hru_order=False, uscutoff_seg=None, verbose=False):
    ...
    dag_ds = nhm_params.stream_network(tosegment='tosegment_nhm', seg_id='nhm_seg')
...
def extract_cbh(ctl, hru_order_subset, st_date=None, en_date=None, cbh_dir=None,
                cbh_netcdf=False, outdir=None, verbose=False):
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    if len(hru_order_subset) > 0:
        ...
        if cbh_netcdf:
            ...
            cbh_hdl.write_ascii(variables=['tmax', 'tmin', 'prcp'])
        # bandit_log.info('{} written to: {}'.format(vv, '{}/{}.cbh'.format(outdir, vv)))
    else:
        bandit_log.error('No HRUs associated with the segments')


def extract_dynamic_parameters(ctl, hru_order_subset, st_date=None, en_date=None,
                               dyn_params_dir=None, outdir=None, verbose=False):
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
def extract_model_output(hru_order_subset, new_nhm_seg, st_date=None, en_date=None,
                         outdir=None, output_vars=None, output_vars_dir=None,
                         verbose=False):
...
def extract_parameters(ctl, nhm_params, required_params, hru_order_subset,
                       new_hru_deplcrv, new_hru_segment,
                       new_nhm_seg, new_poi_gage_id, new_poi_gage_segment,
                       new_poi_type, new_tosegment, nhm_global_dimensions,
                       uniq_deplcrv0, no_filter_params=False, verbose=False):
    ...
    new_params.sort()
    for pp in params:
        if pp in new_params or no_filter_params:
            ...
            if ndims == 1:
                ...
                if first_dimension == 'one':
                    ...
                    # This is really a 2D in disguise, however, it is stored in C-order unlike the
                    # other 2D arrays
...
def extract_shapefiles(hru_order_subset, new_nhm_seg, geo_file=None, outdir=None,
                       hru_gis_id=None, hru_gis_layer=None,
                       seg_gis_id=None, seg_gis_layer=None, verbose=False):
    if verbose:
        print('-'*40)
...
def extract_streamflow(new_poi_gage_id, st_date=None, en_date=None, obs_filename=None,
...
def subset_hru_deplcrv(nhm_params, hru_order_subset):
    # ==========================================================================
    ...
    return uniq_deplcrv0, new_hru_deplcrv


def subset_pois(nhm_params, new_nhm_seg, new_nhm_seg_to_idx1, seg_to_hru, addl_gages=None):
    # @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
def main():
    ...
    # ========================================
    model_extraction(nhm_params=nhm_params, check_dag=check_dag,
                     dsmost_seg=dsmost_seg, hru_noroute=hru_noroute,
                     keep_hru_order=args.keep_hru_order, uscutoff_seg=uscutoff_seg,
                     verbose=args.verbose)
    # Need from model_extraction()
    # hru_order_subset
    # new_hru_deplcrv
    # new_hru_segment
    # new_nhm_seg
    ...
    uniq_deplcrv0, new_hru_deplcrv = subset_hru_deplcrv(nhm_params=nhm_params,
                                                        hru_order_subset=hru_order_subset)

    subset_pois(nhm_params=nhm_params, new_nhm_seg=new_nhm_seg,
                new_nhm_seg_to_idx1=new_nhm_seg_to_idx1, seg_to_hru=seg_to_hru,
    ...
    new_ps = extract_parameters(ctl=ctl, nhm_params=nhm_params, required_params=required_params,
                                hru_order_subset=hru_order_subset,
                                new_hru_deplcrv=new_hru_deplcrv, uniq_deplcrv0=uniq_deplcrv0,
                                new_hru_segment=new_hru_segment, new_nhm_seg=new_nhm_seg,
                                new_poi_gage_id=new_poi_gage_id, new_poi_gage_segment=new_poi_gage_segment,
                                new_poi_type=new_poi_type, new_tosegment=new_tosegment,
                                nhm_global_dimensions=nhm_global_dimensions,
                                no_filter_params=args.no_filter_params, verbose=args.verbose)

    # Write the new parameter file
    header = [f'Written by Bandit version {__version__}', f'ParamDb revision: {git_url}']
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    if output_cbh:
        extract_cbh(ctl=ctl, hru_order_subset=hru_order_subset,
                    st_date=st_date, en_date=en_date, cbh_dir=args.cbh_dir,
                    cbh_netcdf=args.cbh_netcdf, outdir=outdir, verbose=args.verbose)
    ...
    if include_model_output:
        extract_model_output(hru_order_subset=hru_order_subset, new_nhm_seg=new_nhm_seg,
                             st_date=st_date, en_date=en_date, outdir=outdir,
                             output_vars=output_vars, output_vars_dir=output_vars_dir,
                             verbose=args.verbose)
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    if ctl.has_dynamic_parameters:
        extract_dynamic_parameters(ctl=ctl, hru_order_subset=hru_order_subset,
                                   st_date=st_date, en_date=en_date,
                                   dyn_params_dir=dyn_params_dir, outdir=outdir,
                                   verbose=args.verbose)
    ...
    if output_streamflow:
        extract_streamflow(new_poi_gage_id=new_poi_gage_id, st_date=st_date, en_date=en_date,
                           obs_filename=obs_filename, outdir=outdir, verbose=args.verbose)

    # *******************************************
    # Create a shapefile of the selected HRUs
    if output_shapefiles:
        extract_shapefiles(hru_order_subset=hru_order_subset, new_nhm_seg=new_nhm_seg,
                           geo_file=geo_file, outdir=outdir, hru_gis_id=args.hru_gis_id,
                           hru_gis_layer=args.hru_gis_layer, seg_gis_id=args.seg_gis_id,
                           seg_gis_layer=args.seg_gis_layer, verbose=args.verbose)
