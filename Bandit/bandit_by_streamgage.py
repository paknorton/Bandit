...
def main():
    ...
    parser = argparse.ArgumentParser(description='Extract model subsets from the National ' +
                                                 'Hydrologic Model')
    ...
    # NOTE: segment cutoffs are not allowed in by-streamgage extractions
    uscutoff_seg = []
    ...
    # NOTE: nhm_seg is not needed for the by-streamgage bandit run.
    ...
    for sg in streamgages:
        print(f'Working on streamgage {sg}')
        bandit_log.info(f'Streamgage {sg}')
        ...
        try:
            ...
            if dsmost_seg[0] == 0:
                ...
                bandit_log.error(f'Streamgage {sg} has segment = 0; skipping.')
                continue
                # break
            elif len(dsmost_seg) > 1:
                # Should never have more than one segment per streamgage
                bandit_log.info(f'Streamgage {sg} has more than one stream segment; skipping.')
                continue
                # break
        except KeyError:
            bandit_log.error(f'Streamgage {sg} does not exist in poi_gage_id; skipping')
            continue
            # break
        ...
        if ctl.has_dynamic_parameters:
            ...
            for cparam in ctl.dynamic_parameters:
                ...
                output_file = f'{sg_dir}/{param_name}.param'
        ...
        if output_shapefiles:
            ...
            if not os.path.isdir(geo_file):
                ...
                geo_shp.write_shapefile(f'{sg_dir}/GIS/HRU_subset.shp', args.hru_gis_id, hru_order_subset,
                                        included_fields=['nhm_id', 'model_idx', args.hru_gis_id])

                geo_shp.select_layer(args.seg_gis_layer)
                geo_shp.write_shapefile(f'{sg_dir}/GIS/Segments_subset.shp', args.seg_gis_id, new_nhm_seg,
